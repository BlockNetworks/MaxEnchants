...
class MaxEnchants extends PluginBase implements Listener {
    ...
    public function onEnable() {
        ...
        if (is_null($this->cmdName) || $this->cmdName == "") {
            ...
            $cmdInstance = $this->getServer()->getCommandMap()->getCommand($this->cmdName)
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool {
        if ($command->getName() == "maxenchants") {
            $sender->sendMessage(TextFormat::GRAY . "---" . TextFormat::GOLD . " MaxEnchants " . TextFormat::GRAY . "---");
            $sender->sendMessage(TextFormat::YELLOW . "Version: " . TextFormat::AQUA . $this->pluginVersion);
            $sender->sendMessage(TextFormat::YELLOW . "Description: " . TextFormat::AQUA . "Enable enchants up to level 255");
            $sender->sendMessage(TextFormat::YELLOW . "Command: " . TextFormat::BLUE . "/" . $this->cmdName . " <player> <enchantment ID> [level]");
    ...
    private function enchantItem(CommandSender $sender, array $args): void {
        if (count($args) < 2 || count($args) > 3) {
            $sender->sendMessage(TextFormat::RED . "Usage: /" . $this->cmdName . " <player> <enchantment ID> [level]");
            return;
        }
        ...
        if (($item = $player->getInventory()->getItemInHand())->isNull()) {
            $sender->sendMessage(TextFormat::RED . "Player must be holding an item");
            return;
        }
        ...
        if (!($enchantment instanceof Enchantment)) {
            $sender->sendMessage(TextFormat::RED . "There is no such enchantment with ID " . TextFormat::YELLOW . $args[1]);
            return;
        }
        $level = 1;
        if (isset($args[2])) {
            if (!is_numeric($args[2])) {
                ...
                $sender->sendMessage(TextFormat::RED . "Enchantment level must be greater than " . TextFormat::YELLOW . "1");
                return;
            }
            $max = $this->maxLevel;
            ...
            if ($desiredLevel > $max) {
                $sender->sendMessage(TextFormat::RED . "Enchantment level can be at most " . TextFormat::YELLOW . $max);
                return;
            }
            $level = $args[2];
        }
    ...
    private function broadcast(string $name, string $level, Player $target, $sender): void {
        ...
        if ($include !== "" && !is_null($include) && $include !== []) {
            ...
            if (strpos($include, "console") !== false) {
                $this->getLogger()->info(TextFormat::clean($msgString));
            }
